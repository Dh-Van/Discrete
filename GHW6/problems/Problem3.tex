...
\]
Turning this into more formal summation, we get a final answer that looks like this:
\[
f_t = t^s - \sum_{i=1}^{t-1} \binom{t}{i} f_{t-i}, \quad \text{with } f_1 = 1  \text{ , } s \ge 1 \text{ and }t \ge1
\]
This recurrence starts with $t^s$ and subtracts the over counted cases where one or more of the $t$ elements are not used, matching the pattern we observed from the earlier examples.\\\\
\textbf{Part (c):}\\
